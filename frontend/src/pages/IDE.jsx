...
const PANELS = [
  ...
  { id: 'terminal', icon: <Terminal size={14} />, label: 'Terminal-soon', color: 'text-green' },
  { id: 'sandbox', icon: <BoxIcon size={14} />, label: 'sandbox-soon', color: 'text-red' },
]
...
export default function IDE({ token, onLogout }) {
  ...
  const openGhFile = async (file) => {
    ...
    try {
      ...
      addTab(file.path, content ?? '', sha, false)  // ← null-coalesce here
    } catch (e) { showStatus('Error: ' + e.message) }
  }
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex border-l border-border shrink-0">
          <div className="w-10 flex flex-col items-center py-3 gap-1 bg-default">
            {PANELS.map(p => {
              const isDisabled = p.id === 'terminal' || p.id === 'sandbox';
              return (
                <button key={p.id}
                  onClick={() => {
                    if (isDisabled) return;
                    setActivePanel(p.id);
                    setPanelOpen(prev => activePanel === p.id ? !prev : true);
                  }}
                  title={p.label}
        ...
        ${isDisabled
                      ...
                      : activePanel === p.id && panelOpen
                        ? `bg-subtle ${p.color}`
                        : 'text-fg-subtle hover:bg-subtle hover:text-fg-default'
                    }`}>
                  {p.icon}
                </button>
              );
